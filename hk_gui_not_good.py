...
def unlockAllSpells():
    spellStausCheck = spellStatus.get()
    if spellStausCheck == 1:
        pm.write_int(getPointerAddress(gameModule+0x019B8900, offsets=[0x0, 0xD8, 0x268, 0xC8, 0x25D]), 1)
        pm.write_int(getPointerAddress(gameModule+0x019B8900, offsets=[0x0, 0xD8, 0x268, 0xC8, 0x254]), 4)
        pm.write_int(getPointerAddress(gameModule+0x019B8900, offsets=[0x0, 0xD8, 0x268, 0xC8, 0x260]), 1)
        pm.write_int(getPointerAddress(gameModule+0x019B8900, offsets=[0x0, 0xD8, 0x268, 0xC8, 0x264]), 1)
        pm.write_int(getPointerAddress(gameModule+0x019B8900, offsets=[0x0, 0xD8, 0x268, 0xC8, 0x268]), 1)
        
    else:
        pm.write_int(getPointerAddress(gameModule+0x019B8900, offsets=[0x0, 0xD8, 0x268, 0xC8, 0x25D]), 1)
        pm.write_int(getPointerAddress(gameModule+0x019B8900, offsets=[0x0, 0xD8, 0x268, 0xC8, 0x254]), 4)
        pm.write_int(getPointerAddress(gameModule+0x019B8900, offsets=[0x0, 0xD8, 0x268, 0xC8, 0x260]), 0)
        pm.write_int(getPointerAddress(gameModule+0x019B8900, offsets=[0x0, 0xD8, 0x268, 0xC8, 0x264]), 0)
        pm.write_int(getPointerAddress(gameModule+0x019B8900, offsets=[0x0, 0xD8, 0x268, 0xC8, 0x268]), 0)
...
stagCheck.grid(column=0,padx=10, row=12)

noneStatus = tkinter.IntVar()
noneCheck = ttk.Label(labelframe2,text="")
noneCheck.grid(column=0,padx=10, row=13)

spellStatus = tkinter.IntVar()
spellCheck = ttk.Checkbutton(labelframe2, state="disabled", width=35, text="Unlock All Spells", variable = spellStatus)
spellCheck.grid(column=0,padx=10, row=11)
